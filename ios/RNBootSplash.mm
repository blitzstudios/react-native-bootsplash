...
@implementation RNBootSplashAnimationView {
  ...
  NSDate *_startDate;
  NSTimeInterval _fallbackDuration;
}

- (instancetype)initWithURL:(NSURL *)url durationMs:(NSInteger)durationMs {
  if (self = [super initWithFrame:CGRectZero]) {
    _fallbackDuration = durationMs / 1000.0;
...
- (NSTimeInterval)remainingAnimationTime {
  ...
  if (item != nil) {
    ...
    if (isfinite(duration) && duration > 0 && isfinite(current)) {
      return MAX(0, duration - current);
    }
  }

  // Item duration not ready yet: estimate from when playback started.
  if (_startDate == nil) {
    return _fallbackDuration;
  }

  return MAX(0, _fallbackDuration - [[NSDate date] timeIntervalSinceDate:_startDate]);
}
