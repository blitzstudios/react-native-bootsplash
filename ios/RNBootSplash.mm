...
// Informal protocol implemented by the app's animated splash loading view.
// Lets us wait for the splash animation to finish before dismissing, without
// the library depending on the app's animation implementation.
@protocol RNBSAnimatedLoadingView <NSObject>
- (NSTimeInterval)rnbsRemainingAnimationTime;
@end
...
+ (void)hideAndClearPromiseQueue {
  ...
  // animation to finish playing before dismissing. The loading view (the app's
  // animated splash root view) reports how much playback time remains.
  if (!_forced && [_loadingView respondsToSelector:@selector(rnbsRemainingAnimationTime)]) {
    NSTimeInterval remaining = [(id<RNBSAnimatedLoadingView>)_loadingView rnbsRemainingAnimationTime];
